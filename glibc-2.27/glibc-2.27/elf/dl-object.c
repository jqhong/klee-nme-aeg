...
# define audit_space 0
    ...
    // /* Jiaqi */
    // if (!strcmp(libname, "libc.so.6"))
    // {
    //     size_t tmp_size = sizeof (*new) + audit_space + sizeof (struct link_map *) + sizeof (*newname) + libname_len;
    //     _dl_debug_printf ("\n ~~~~~~~~~~~~~~~~~~~`test~~~~~~~~~~~~` sizeof(new): %lx, realname: %s, libname: %s, type: %x. \n", tmp_size, realname, libname, type);
    //     new = mmap((void*)0x7ffff7ff9000, tmp_size, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS|MAP_FIXED, -1, 0);//use mmap instead of calloc to fix the addr of this 
    //     // new = mmap((void*)0x7ffff7ff9000, 0x2000, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS|MAP_FIXED, -1, 0);//use mmap instead of calloc to fix the addr of this 
