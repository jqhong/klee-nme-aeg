...
tcache_get (size_t tc_idx)
{
    ...
    /* Jiaqi */
    // // // _dl_debug_printf("about to invoke _int_malloc. \n");
    // __fxprintf (NULL, "size of malloc_chunk: %lx. itc_idx: %ld. tcache_entry e: %p, e->next: %p. \n", sizeof(struct malloc_chunk), tc_idx, e, e->next);
    // unsigned long* tmp;
    // int i = 0;
    // tmp = (unsigned long*) e; 
    // for (i = 0 ; i < 5; i ++)
    // {
    //     __fxprintf (NULL, "addr: %p, content: %lx. \n", tmp, *tmp);
    //     tmp --;
    // }
    // /* /Jiaqi */
    assert (tc_idx < TCACHE_MAX_BINS);
...
void * __libc_malloc (size_t bytes)
{
    ...
    /* Jiaqi */
    // // _dl_debug_printf("about to invoke _int_malloc. \n");
    // __fxprintf (NULL, "NFASTBINS: %d. BINMAPSIZE: %d.\n", NFASTBINS, BINMAPSIZE);
    // __fxprintf (NULL, "addr of main_arena: %p, addr of mp_: %p, addr of tcache: %p, %p. \n", &main_arena, &mp_, tcache, &tcache);
    // __fxprintf (NULL, "addr of main_arena->mutex: %p, : %d, addr flags: %p, addr of have_fastchunks: %p. \n", &main_arena.mutex, main_arena.mutex, &main_arena.flags, &main_arena.have_fastchunks);
    // __fxprintf (NULL, "tcache_bins: %d, tcache_max_bytes: %d, tcache_count: %d, tcache_unsorted_limit: %d. \n", (int)mp_.tcache_bins, (int)mp_.tcache_max_bytes, (int)mp_.tcache_count, (int)mp_.tcache_unsorted_limit);
    // mstate tmp_av = &main_arena;
    // __fxprintf (NULL, "__libc_malloc, bytes: %lx. tbytes: %lx, tc_idx: %lx, main_arena: %p.\n", bytes, tbytes, tc_idx, tmp_av);
    // mchunkptr tmp_victim;                 /* inspected/selected chunk */
    // mbinptr bin;                      /* associated bin */
    // int i = 0;
    // for (i = 0; i < 3; i ++)
    // {
    //     mfastbinptr *fb = &fastbin (tmp_av, i);
    //     tmp_victim = *fb;
    //     if (tmp_victim != NULL)
    //     {
    //         __fxprintf (NULL, "fast bin. idx: %d. fb: %p, victim: %p, fb->fd: %p, fb->bk: %p, size: %lx, pre_size: %lx. \n", i, fb, tmp_victim, tmp_victim->fd, tmp_victim->bk, chunksize(tmp_victim), prev_size(tmp_victim));
    //         if (tmp_victim->fd)
    //         {
    //             tmp_victim = tmp_victim->fd;
    //             __fxprintf (NULL, "second, fast bin. idx: %d. fb: %p, victim: %p, fb->fd: %p, fb->bk: %p, size: %lx, pre_size: %lx. \n", i, fb, tmp_victim, tmp_victim->fd, tmp_victim->bk, chunksize(tmp_victim), prev_size(tmp_victim));
    //         }
    //     }
    // }
    // for (i = 0; i < 3; i ++)
    ...
    //     if ((tmp_victim = last (bin)) != bin)
    //     {
    //         /* Jiaqi */
    //         __fxprintf (NULL, "small bin has been initialized. bytes: 0x%lx, idx: %d, bin: %p, victim: %p. \n", bytes, i, bin, last(bin));
    //     }
    // }
    //  __fxprintf (NULL, "unsorted bin: %p,fd: %p, bk: %p. sizeof malloc chunk: %lx. \n", unsorted_chunks (&main_arena), unsorted_chunks (&main_arena)->fd, unsorted_chunks (&main_arena)->bk, sizeof(struct malloc_chunk));
    // mbinptr tmp_bin = bin_at (&main_arena, 1);
    // __fxprintf (NULL, "small bin: %p,fd: %p, bk: %p. \n", tmp_bin, first(tmp_bin), last(tmp_bin));
    // tmp_bin = bin_at (&main_arena, 2);
    // __fxprintf (NULL, "small bin: %p,fd: %p, bk: %p. \n", tmp_bin, first(tmp_bin), last(tmp_bin));
    // tmp_bin = bin_at (&main_arena, 3);
    // __fxprintf (NULL, "small bin: %p,fd: %p, bk: %p. \n", tmp_bin, first(tmp_bin), last(tmp_bin));
        // mfastbinptr *fb = &fastbin (av, idx);
        // mchunkptr pp;
        // victim = *fb;
    // mfastbinptr *tmpp_bin = &fastbin (&main_arena, 1);
    // if (tmpp_bin)
    // {
    //     tmp_bin = *tmpp_bin;
    //     __fxprintf (NULL, "fast bin: %p,fd: %p, bk: %p. \n", tmp_bin, first(tmp_bin), last(tmp_bin));
    // }
    // tmpp_bin = &fastbin (&main_arena, 2);
    // if (tmpp_bin)
    // {
    //     tmp_bin = *tmpp_bin;
    //     __fxprintf (NULL, "fast bin: %p,fd: %p, bk: %p. \n", tmp_bin, first(tmp_bin), last(tmp_bin));
    ...
    if (SINGLE_THREAD_P)
    {
        /* Jiaqi */
        // // _dl_debug_printf("about to invoke _int_malloc. \n");
        // __fxprintf (NULL, "about to invoke _int_malloc.\n");
        // /* /Jiaqi */
...
// /* Jiaqi */
// void* n_malloc (mstate av, size_t bytes, void* n_ptr)
// {
...
_int_malloc (mstate av, size_t bytes)
{
    ...
    // /* Jiaqi */
    // __fxprintf (NULL, ">>>>>>>>>>>>>>>>>>>>>>>>>>_int_malloc. bytes: %lx, nb: %lx. \n", bytes, nb);
    // __fxprintf (NULL, "main_arena: %p. have fast bin: %d. bytes: %lx, nb: %lx. \n", av, av->have_fastchunks, bytes, nb);
    // __fxprintf (NULL, "unsorted bin: %p,fd: %p, bk: %p. \n", unsorted_chunks (av), unsorted_chunks (av)->fd, unsorted_chunks (av)->bk);
    // mbinptr tmp_bin = bin_at (av, 2);
    // __fxprintf (NULL, "small bin: %p,fd: %p, bk: %p. \n", tmp_bin, first(tmp_bin), last(tmp_bin));
    // /* /Jiaqi */
...
#define REMOVE_FB(fb, victim, pp)			\
    ...
    if ((unsigned long) (nb) <= (unsigned long) (get_max_fast ()))
    {
        ...
        victim = *fb;
        /* Jiaqi */
        // __fxprintf (NULL, "nb: 0x%lx, have fast bin: %d. bytes: 0x%lx, get_max_fast(): 0x%lx, idx: %d, fb: %p, victim: %p. \n", nb, av->have_fastchunks, bytes, get_max_fast(), idx, fb, victim);
        // __fxprintf (NULL, "unsorted bin: %p,fd: %p, bk: %p. \n", unsorted_chunks (av), unsorted_chunks (av)->fd, unsorted_chunks (av)->bk);
        // if (victim != NULL)
        // {
        //     __fxprintf (NULL, "fast bin. idx: %d. fb: %p, victim: %p, fb->fd: %p, fb->bk: %p, size: %lx, pre_size: %lx. \n", idx, fb, victim, victim->fd, victim->bk, chunksize(victim), prev_size(victim));
        // }
        // /* /Jiaqi */
        ...
        {
            /* Jiaqi */
            // __fxprintf (NULL, "fast bin has been initialized. idx: %d. fb: %p, victim: %p, fb->fd: %p, fb->bk: %p, size: %lx, pre_size: %lx. \n", idx, fb, victim, victim->fd, victim->bk, chunksize(victim), prev_size(victim));
            /* /Jiaqi */
            if (SINGLE_THREAD_P)
                *fb = victim->fd;
            else
    ...
    else
    {
        ...
        // /* Jiaqi */
        // __fxprintf (NULL, "have fast bin: %d. bytes: %lx, nb: %lx. \n", av->have_fastchunks, bytes, nb);
        // __fxprintf (NULL, "large bin. nb: 0x%lx. \n", nb);
        // /* /Jiaqi */
    ...
    for (;; )
    {
        ...
        // /* Jiaqi */
        // __fxprintf (NULL, "have fast bin: %d. bytes: %lx, nb: %lx. \n", av->have_fastchunks, bytes, nb);
        // __fxprintf (NULL, "for loop. unsorted bin: %p,fd: %p, bk: %p. \n", unsorted_chunks (av), unsorted_chunks (av)->fd, unsorted_chunks (av)->bk);
        // /* /Jiaqi */
        while ((victim = unsorted_chunks (av)->bk) != unsorted_chunks (av))
        {
            // /* Jiaqi */
            // __fxprintf (NULL, "have fast bin: %d. bytes: %lx, nb: %lx. \n", av->have_fastchunks, bytes, nb);
            // __fxprintf (NULL, "while loop. unsorted bin: %p,fd: %p, bk: %p. \n", unsorted_chunks (av), unsorted_chunks (av)->fd, unsorted_chunks (av)->bk);
            // /* /Jiaqi */
            bck = victim->bk;
...
#define MAX_ITERS       10000
        ...
        {
            /* Jiaqi */
            // __fxprintf (NULL, "!in_smallbin_range. \n");
            /* /Jiaqi */
            bin = bin_at (av, idx);
        ...
        {
            /* Jiaqi */
            // __fxprintf (NULL, "search for a chunk. idx:%d, bin:%p, block: %d. bit: %d, map: %d. \n", idx, bin, block, map, bit);
            /* /Jiaqi */
...
use_top:
        ...
        /* Jiaqi */
        // __fxprintf (NULL, "use_top. \n");
        /* /Jiaqi */

        if ((unsigned long) (size) >= (unsigned long) (nb + MINSIZE))
        {
            /* Jiaqi */
            // __fxprintf (NULL, "size>=nb. \n");
            /* /Jiaqi */
            remainder_size = size - nb;
            remainder = chunk_at_offset (victim, nb);
            av->top = remainder;
        ...
        {
            /* Jiaqi */
            // __fxprintf (NULL, "atomiz_load_relaxed. \n");
            /* /Jiaqi */
            malloc_consolidate (av);
