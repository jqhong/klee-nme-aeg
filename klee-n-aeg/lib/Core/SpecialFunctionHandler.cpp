...
void SpecialFunctionHandler::handleNew(ExecutionState &state,
    ...
    bool isRuntimeLib = 0;
    bool isUclibc = 0;
    std::string runtime_path = "runtime";
    std::string uclibc_path = "lib";
    const char* runtimestr = runtime_path.c_str();
    const char* uclibcstr = uclibc_path.c_str();
    // printf ("----------------file_name.size(): %d, isRuntimeLib: %d. isUclibc: %d. \n", file_name.size(), isRuntimeLib, isUclibc);
    // printf ("runtime_path: %s. \n", runtimestr);
    // printf ("uclibc_path: %s. \n", uclibcstr);
    // printf ("filestr: %p, runtimestr: %p, strstr result: %p. \n", filestr, runtimestr, strstr(filestr, runtimestr));
    if (file_name.size() != 0)
    {
        if (strstr(filestr, runtimestr) == filestr)
        // if (strstr(filestr, runtimestr) != NULL)
        {
            isRuntimeLib = 1;
            // printf ("/////////////////////////////set runtime as 1. \n");
        }
        if (strstr(filestr, uclibcstr) == filestr)
        // if (strstr(filestr, uclibcstr) != NULL)
        {
            isUclibc = 1;
        }
    }
    // printf ("file_name.size(): %d, isRuntimeLib: %d. isUclibc: %d. \n", file_name.size(), isRuntimeLib, isUclibc);
    if (file_name.size() == 0 || isRuntimeLib == 1 || isUclibc == 1)
    {
        printf ("normal executeAlloc invoked. \n");
        executor.executeAlloc(state, arguments[0], false, target);
    }
    else
    {
        printf ("native executeAlloc invoked. \n");
        executor.executeAlloc(state, arguments[0], false, target, false,  nullptr, 0, 1);
    }
    /* /Jiaqi */
}
...
void SpecialFunctionHandler::handleMalloc(ExecutionState &state,
    ...
    // // os << target->getSourceLocation() <<;
    // // std::string msg("malloc called from");
...
void SpecialFunctionHandler::handleCalloc(ExecutionState &state,
    ...
    printf ("handleCalloc invoked in state: %p. \n", &state);
    std::string file_name = target->info->file;
    const char* filestr = file_name.c_str();
    bool isRuntimeLib = 0;
    bool isUclibc = 0;
    ...
    if (file_name.size() == 0 || isRuntimeLib == 1 || isUclibc == 1)
        ...
        executor.executeAlloc(state, size, false, target, true, nullptr, 0, 1);
    /* /Jiaqi */
}
